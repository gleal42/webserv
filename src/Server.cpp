...
Server::CreateError::CreateError(void)
	: std::runtime_error("Failed to create Kernel Queue.")
{ /* No-op */
}

// Constructors
Server::Server(void) : _queue_fd(FD_UNSET), _sigint(false)
{ /* No-op */
}
	...
	while ((de = readdir(dir)) != NULL)
	{
		...
		if (lstat(file_path.c_str(), &st) == 0)
		{
			...
			if (is_dir)
			{
				...
				dirs_buffer += file_name + "</a>" + insert_whitespace(file_name.length(), WHITESPACE_CAP) + set_time(&tm_time) + insert_whitespace(1, WHITESPACE_CAP / 2) + "-</div>";
			}
			else
			{
				...
				files_buffer += file_name + "</a>" + insert_whitespace(file_name.length(), WHITESPACE_CAP) + set_time(&tm_time) + insert_whitespace(fsize.length(), WHITESPACE_CAP / 2) + fsize + "</div>";
			}
		}
	}
	closedir(dir);
...
ServerConfig Server::config_resolve(const Request &req, Response &res, const in_addr &connection_addr)
{
	...
	{
		if (is_address_being_listened(it->second->_config.get_ip(), (const sockaddr_in *)&connection_addr) && it->second->_config.get_port() == req.request_uri.port)
		{
